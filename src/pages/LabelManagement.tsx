...
.label-card{background:#fff;border:2px solid #cbd5e1;border-radius:10px;padding:24px 32px;display:flex;gap:24px;align-items:center;box-shadow:0 8px 32px rgba(0,0,0,0.12);transform:scale(1.8);transform-origin:center}
.label-logos{display:flex;flex-direction:column;gap:10px;align-items:center}
...
<div class="sidebar">
  ...
  <div class="section">
    ...
    <div class="design-controls">
      ...
      <div class="control-row"><span class="control-label">Barcode Width</span><input type="number" class="control-input" id="barcodeWidth" value="1.5" min="1" max="3" step="0.1" oninput="updateDesign()"><span class="control-unit"></span></div>
      <div class="control-row"><span class="control-label">Barcode Height</span><input type="number" class="control-input" id="barcodeHeight" value="35" min="20" max="60" oninput="updateDesign()"><span class="control-unit">px</span></div>
...
<div class="main">
  ...
  <div class="preview-container">
    <div class="preview-header"><span class="preview-label">📋 Live Preview (scaled 1.8x)</span><span class="preview-label">50×25MM</span></div>
    <div class="preview-body">
      <div class="label-card" id="labelCard">
        <div class="label-logos" id="logoSection">
          <img id="logoUp" src="label-logo-up.png" alt="" onerror="this.style.display='none'" style="width:72px;height:auto;max-height:55px"/>
          <img id="logoDown" src="label-logo-down.png" alt="" onerror="this.style.display='none'" style="width:72px;height:auto;max-height:55px"/>
        </div>
...
let design={logoWidth:18,barcodeWidth:1.5,barcodeHeight:35,textSize:7,priceSize:14};

function initBarcode(){
  JsBarcode("#barcode",barcodeText,{format:"CODE128",width:design.barcodeWidth,height:design.barcodeHeight,displayValue:false,margin:0,background:"#ffffff",lineColor:"#000000"});
}
initBarcode();

function updateDesign(){
  design.logoWidth=parseFloat(document.getElementById('logoWidth').value)||18;
  design.barcodeWidth=parseFloat(document.getElementById('barcodeWidth').value)||2;
  design.barcodeHeight=parseInt(document.getElementById('barcodeHeight').value)||40;
  design.textSize=parseInt(document.getElementById('textSize').value)||8;
  design.priceSize=parseInt(document.getElementById('priceSize').value)||16;
  
  document.getElementById('logoUp').style.width=design.logoWidth*4+'px';
  document.getElementById('logoDown').style.width=design.logoWidth*4+'px';
      ...
      for (const label of labels) {
        for (let i = 0; i < label.quantity; i++) {
          ...
          const result = await window.electronAPI.printLabelWithImage(
            ...
            label.price,
            printerName,
            design  // Pass design settings to the print handler
          );
