...
async function getDBStatistics() {
  const supabase2 = getSupabase();
  if (!isSupabaseConfigured() || !supabase2) {
    return {
      totalSize: 500,
      usedSize: 0,
      freeSize: 500,
  ...
  try {
    const { count: productsCount } = await supabase2.from("products_sync").select("*", { count: "exact", head: true }).eq("store_id", STORE_ID);
    const { count: billsCount } = await supabase2.from("bills_sync").select("*", { count: "exact", head: true }).eq("store_id", STORE_ID);
    const { count: billItemsCount } = await supabase2.from("bill_items_sync").select("*", { count: "exact", head: true }).eq("store_id", STORE_ID);
    const estimatedSize = (productsCount || 0) * 1 + (billsCount || 0) * 2 + (billItemsCount || 0) * 0.5;
    const usedSizeMB = estimatedSize / 1024;
    const totalSizeMB = 500;
    const freeSizeMB = totalSizeMB - usedSizeMB;
    const bandwidthUsedMB = usedSizeMB * 2;
    const bandwidthLimitMB = 5e3;
    return {
...
async function syncSingleBill(queueId) {
  const supabase2 = getSupabase();
  if (!isSupabaseConfigured() || !supabase2) {
    throw new Error("Supabase not configured");
  }
  const db2 = getDatabase();
  const queueStmt = db2.prepare("SELECT * FROM sync_queue WHERE id = ?");
  const queueItem = queueStmt.get(queueId);
  if (!queueItem) {
    throw new Error("Queue item not found");
  }
  const item = {
    ...queueItem,
    data: queueItem.data ? JSON.parse(queueItem.data) : void 0
  };
  try {
    await pushSyncItem(item);
    markAsSynced(queueId);
    if (item.table_name === "bills") {
      const billItemsStmt = db2.prepare(`
        SELECT * FROM sync_queue 
        WHERE table_name = 'bill_items' 
        AND synced = 0 
        AND json_extract(data, '$.bill_id') = ?
      `);
      const billItems = billItemsStmt.all(item.record_id);
      for (const billItem of billItems) {
        const parsedItem = {
          ...billItem,
          data: billItem.data ? JSON.parse(billItem.data) : void 0
        };
    ...
    markSyncFailed(queueId, error.message);
    throw error;
  }
}
function createBill(data) {
  ...
  try {
    const { recordBillPaymentTransactions: recordBillPaymentTransactions2 } = require("./cashUpiTransactions");
    recordBillPaymentTransactions2(
      billNumber,
      data.paymentMode,
      data.cashAmount || 0,
      data.upiAmount || 0
...
async function checkNetworkStatus() {
  try {
    ...
    networkStatus.lastChecked = (/* @__PURE__ */ new Date()).toISOString();
    if (!isOnline) {
...
async function measureActualSpeed() {
  try {
    ...
    const timeTakenSeconds = (endTime - startTime) / 1e3;
...
function isFirebaseConfigured() {
  return !!(process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_PRIVATE_KEY && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_STORAGE_BUCKET);
}
...
`;
  ...
  ipcMain.handle("sync:syncSingleBill", async (_, queueId) => {
    try {
  ...
  ipcMain.handle("stores:getStores", async () => {
    ...
    try {
      ...
      const stores = [
        {
          id: config2.storeId || "store_001",
          name: config2.storeName || "Main Store",
          type: config2.storeType || "master",
      ...
      if ((_a = config2.sync) == null ? void 0 : _a.branchStores) {
        for (const branch of config2.sync.branchStores) {
          stores.push({
            id: branch.storeId,
  ...
  ipcMain.handle("stores:getStoreStats", async (_, storeId) => {
    try {
      ...
      const mockStats = [
        ...
        },
        {
          ...
          pendingBills: 8,
          syncedBills: 372,
          lastSyncTime: new Date(Date.now() - 72e5).toISOString()
        }
      ];
  ...
  ipcMain.handle("stores:pullStoreData", async (_, storeId) => {
    try {
      const { getSupabase: getSupabase2 } = await Promise.resolve().then(() => supabaseClient);
      const { getDatabase: getDatabase2 } = await Promise.resolve().then(() => connection);
      const { getConfig: getConfig2 } = await Promise.resolve().then(() => config$2);
      ...
      const { data: lastSyncData } = await supabase2.from("sync_metadata").select("last_pull_at").eq("store_id", storeId).eq("table_name", "bills").single();
      const lastSync = lastSyncData == null ? void 0 : lastSyncData.last_pull_at;
      let query = supabase2.from("bills_sync").select("*").eq("store_id", storeId);
      if (lastSync) {
      ...
      if (bills && bills.length > 0) {
        ...
        await supabase2.from("sync_metadata").upsert({
          store_id: storeId,
          table_name: "bills",
          last_pull_at: now,
          updated_at: now
      ...
      return {
        ...
        storeId,
        recordsPulled,
        lastPullTime: (/* @__PURE__ */ new Date()).toISOString()
      };
    } catch (error) {
  ...
  ipcMain.handle("stores:getSupabaseStores", async () => {
    try {
      ...
      const { data: stores, error } = await supabase2.from("stores").select("*").order("created_at", { ascending: true });
      if (error) {
        throw error;
      }
      return stores || [];
  ...
  ipcMain.handle("stores:getLocalBillCount", async (_, storeId) => {
    try {
      const { getDatabase: getDatabase2 } = await Promise.resolve().then(() => connection);
      const { getConfig: getConfig2 } = await Promise.resolve().then(() => config$2);
      const db2 = getDatabase2();
      ...
      const currentStoreId = config2.storeId || "store_001";
      if (storeId === currentStoreId) {
        const stmt = db2.prepare("SELECT COUNT(*) as count FROM bills");
        const result2 = stmt.get();
        return result2.count;
      } else {
        return 0;
  ...
  ipcMain.handle("stores:getSupabaseBillCount", async (_, storeId) => {
    try {
      const { getSupabase: getSupabase2 } = await Promise.resolve().then(() => supabaseClient);
      const supabase2 = getSupabase2();
      if (!supabase2) {
      ...
      const { count, error } = await supabase2.from("bills_sync").select("*", { count: "exact", head: true }).eq("store_id", storeId);
      if (error) {
        throw error;
      }
      return count || 0;
  ...
  ipcMain.handle("stores:getLastSyncTime", async (_, storeId) => {
    try {
      const { getSupabase: getSupabase2 } = await Promise.resolve().then(() => supabaseClient);
      const supabase2 = getSupabase2();
      if (!supabase2) {
      ...
      const { data: metadata, error } = await supabase2.from("sync_metadata").select("last_pull_at").eq("store_id", storeId).eq("table_name", "bills").single();
      if (error || !metadata) {
        return null;
      }
  ...
  ipcMain.handle("cashUpi:addTransaction", async (_, data) => {
    try {
      const { addCashUpiTransaction } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:getTransactions", async (_, type, transactionType, dateFrom, dateTo, limit, offset) => {
    try {
      const { getCashUpiTransactions } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:getTransactionById", async (_, id) => {
    try {
      const { getCashUpiTransactionById } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:getSummary", async (_, dateFrom, dateTo) => {
    try {
      const { getCashUpiSummary } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:getDailySummary", async (_, date) => {
    try {
      const { getDailyCashUpiSummary } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:updateTransaction", async (_, id, updates) => {
    try {
      const { updateCashUpiTransaction } = await import("./cashUpiTransactions-DdsHpRin.js");
  ...
  ipcMain.handle("cashUpi:recordBillPayment", async (_, billNumber, paymentMode, cashAmount, upiAmount) => {
    try {
      const { recordBillPaymentTransactions } = await import("./cashUpiTransactions-DdsHpRin.js");
