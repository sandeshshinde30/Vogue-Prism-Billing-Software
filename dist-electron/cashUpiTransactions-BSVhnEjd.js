import { g as getDatabase, a as addActivityLog } from "./main-C704uQ6d.js";
function addCashUpiTransaction(data) {
  const db = getDatabase();
  const stmt = db.prepare(`
    INSERT INTO cash_upi_transactions (type, transactionType, amount, reason, description, referenceNumber, createdBy)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  addActivityLog(
    ...
    "cash_upi",
    transactionId,
    `${data.transactionType === "incoming" ? "Added" : "Deducted"} ₹${data.amount} - ${data.reason}`
  );
